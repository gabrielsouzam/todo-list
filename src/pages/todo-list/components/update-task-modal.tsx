...
import { convertFromIsoFormat } from "@/utils/convert-from-iso-format"
...
export function UpdateTaskModal({ task, onTaskUpdated }: UpdateTaskModalProps) {
  ...
  const { date: initialDate, time: initialTime } = convertFromIsoFormat(task.deadline)

  const { register, handleSubmit, formState: { errors }, control } = useForm<UpdateTaskForm>({
  ...
  async function onUpdateTask(data: UpdateTaskForm) {
